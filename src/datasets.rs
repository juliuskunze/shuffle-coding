...
use crate::graph::{AsEdges, DiGraph, EdgeIndex, UnGraph};
use crate::labelled_graph::{EdgeLabelledGraph, NodeLabelledGraph};
...
/// except graph labels are not required.
#[derive(Debug, Clone)]
...
impl SourceInfo {
    pub fn new(name: &str, index_url: &str, hompage_url: Option<&str>) -> Self {
        Self { name: name.to_string(), index_url: index_url.to_string(), homepage_url: hompage_url.map(|x| x.to_string()) }
    }

    pub fn defaults() -> Vec<SourceInfo> {
        lazy_static! {static ref D: Vec<SourceInfo> = vec![
            SourceInfo::new(
                "TU",
                "https://raw.githubusercontent.com/chrsmrrs/datasets/gh-pages/_docs/datasets.md",
                Some("https://chrsmrrs.github.io/datasets/")),
            SourceInfo::new(
                "SZIP",
                "https://raw.githubusercontent.com/juliuskunze/szip-graphs/main/datasets.md",
                Some("https://github.com/juliuskunze/szip-graphs")),
        ];}
        D.clone()
    }
}
...
impl Dataset {
    ...
    pub fn edge_labelled_graphs(&self) -> Option<Vec<EdgeLabelledGraph<Undirected>>> {
        ...
        let graphs_with_edge_order = if self.has_node_labels {
            ...
            self.graphs_with_edge_order().into_iter().map(|(x, es)| (
                NodeLabelledGraph::new(repeat(0).take(x.len()), x.edges()), es)).collect()
        };
    ...
    fn graphs_with_edge_order(&self) -> Vec<(UnGraph, Vec<EdgeIndex>)> {
        ...
        let nodes_by_graph = BTreeMap::from_iter(
            graphs_by_node.iter().enumerate().map(|(i, g)| (*g, i)).into_group_map().into_iter());
        let mut edges_by_graph: HashMap<usize, Vec<_>> = HashMap::from_iter(nodes_by_graph.keys().map(|&g| (g, vec![])));
        for edge in edges {
        ...
        let graphs = nodes_by_graph.into_iter().map(|(g, nodes)| {
            ...
            let dedup = es.iter().cloned().collect::<HashSet<_>>();
...
fn download_files(downloads: &Vec<Download>) {
...
pub fn datasets_from_at(indices: impl IntoIterator<Item=SourceInfo>, cache: &Path) -> Vec<DatasetInfo> {
    ...
    download_files(&indices.iter().filter_map(|s| s.index_download()).collect());
...
mod tests {
    ...
    use crate::ans::{Message, VecCodec};
    use crate::benchmark::{DatasetStats, test_and_print};
    use crate::datasets::dataset;
    use crate::graph_ans::{EmptyCodec, erdos_renyi_indices, GraphIID, polya_urn};
    use crate::graph_ans::tests::test_graph_shuffle_codecs;
    use crate::labelled_graph::tests::{with_edge_labelled_uniform, with_node_labelled_uniform};
    use crate::permutable::{GroupPermutable, Permutable, Unordered};
    use crate::shuffle_ans::test::test_shuffle_codecs;
    ...
    fn test_polyas_urn() {
        ...
        for loops in [false, true] {
            for redraws in [false, true] {
                ...
                test_shuffle_codecs(&pu, unordered, &TestConfig::test(0));
                println!()
            }
        }
    }
